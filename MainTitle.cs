...
public class MainTitle
{
	...
	private float _allyScrollOffset = 0.0f; // 我方角色滚动条偏移量
	private float _enemyScrollOffset = 0.0f; // 敌方角色滚动条偏移量
	private bool _isDraggingAllyScrollBar = false; // 是否正在拖动我方滚动条
	private bool _isDraggingEnemyScrollBar = false; // 是否正在拖动敌方滚动条
	private List<Character> _selectedAllies = new List<Character>(); // 已选择的本方角色列表
	...
	// 新增：返回按钮点击类型
	public enum ButtonType { None, StartGame, Encyclopedia, Tutorial }
	public ButtonType LastClickedButton { get; private set; }
	...
	public MainTitle(SpriteBatch spriteBatch, SpriteFont font, SpriteFont chineseFont, Texture2D pixel)
	{
		...
		_pixel = pixel;
		LastClickedButton = ButtonType.None;
	}
	...
	public void Draw(int windowWidth, int windowHeight)
	{
		...
		// 排序我方角色：示例角色固定在顶部，然后按势力优先级，同一势力按名称排序
		var sortedAllyCharacters = allyCharacterData.OrderBy(c => c.Name.StartsWith("示例") ? 0 : 1).ThenBy(c =>
		{
		...
		foreach (var faction in new[] { Faction.无, Faction.魏, Faction.蜀, Faction.吴, Faction.群 })
		{
			...
			for (int i = 0; i < characters.Count; i++)
			{
				...
				// 计算内容区域：左边缘的右边20像素，滚动条左边缘的左边20像素
		...
		// 游戏百科按钮（左侧）、开始战斗按钮（中间）、教程入口按钮（右侧）
		int menuButtonWidth = 180;
		int menuButtonHeight = 50;
		int menuButtonSpacing = 40;
		int totalMenuButtonsWidth = menuButtonWidth * 3 + menuButtonSpacing * 2;
		int menuButtonsStartX = (windowWidth - totalMenuButtonsWidth) / 2;
		int menuButtonY = scrollAreaY + scrollAreaHeight + 40;
		
		// 游戏百科按钮
		_spriteBatch.Draw(_pixel, new Rectangle(menuButtonsStartX, menuButtonY, menuButtonWidth, menuButtonHeight), Color.Blue);
		string encyclopediaText = "游戏百科";
		SpriteFont encyclopediaFont = GetFontForText(encyclopediaText);
		Vector2 encyclopediaTextSize = encyclopediaFont.MeasureString(encyclopediaText);
		_spriteBatch.DrawString(encyclopediaFont, encyclopediaText, new Vector2(menuButtonsStartX + (menuButtonWidth - encyclopediaTextSize.X) / 2, menuButtonY + (menuButtonHeight - encyclopediaTextSize.Y) / 2), Color.White);
		
		// 开始战斗按钮
		int startButtonX = menuButtonsStartX + menuButtonWidth + menuButtonSpacing;
		_spriteBatch.Draw(_pixel, new Rectangle(startButtonX, menuButtonY, menuButtonWidth, menuButtonHeight), Color.Green);
		...
		_spriteBatch.DrawString(startFont, startText, new Vector2(startButtonX + (menuButtonWidth - startTextSize.X) / 2, menuButtonY + (menuButtonHeight - startTextSize.Y) / 2), Color.White);
		
		// 教程入口按钮
		int tutorialButtonX = menuButtonsStartX + menuButtonWidth * 2 + menuButtonSpacing * 2;
		_spriteBatch.Draw(_pixel, new Rectangle(tutorialButtonX, menuButtonY, menuButtonWidth, menuButtonHeight), Color.Orange);
		string tutorialText = "教程入口";
		SpriteFont tutorialFont = GetFontForText(tutorialText);
		Vector2 tutorialTextSize = tutorialFont.MeasureString(tutorialText);
		_spriteBatch.DrawString(tutorialFont, tutorialText, new Vector2(tutorialButtonX + (menuButtonWidth - tutorialTextSize.X) / 2, menuButtonY + (menuButtonHeight - tutorialTextSize.Y) / 2), Color.White);
		...
		Vector2 slotTitlePosition = new Vector2((windowWidth - slotTitleSize.X) / 2, scrollAreaY + scrollAreaHeight + menuButtonHeight + 60);
		...
		// 计算最多可以选择的行动槽数量
		int selectedAllyCount = _selectedAllies.Count > 0 ? _selectedAllies.Count : 1; // 未选择时视为1个
		int selectedEnemyCount = _selectedEnemies.Count > 0 ? _selectedEnemies.Count : 1; // 未选择时视为1个
	...
	public bool CheckClick(MouseState currentMouseState, int windowWidth, int windowHeight)
	{
		...
		;
			...
			var sortedAllyCharacters = allyCharacterData
				.OrderBy(c => c.Name.StartsWith("示例") ? 0 : 1).ThenBy(c =>
				{
...
("示例敌怪1", () => new Characters.Enemies.示例敌怪1()),
			...
			// 游戏百科按钮
			if (IsPointInRectangle(currentMouseState.X, currentMouseState.Y, menuButtonsStartX, menuButtonY, menuButtonWidth, menuButtonHeight))
			{
				LastClickedButton = ButtonType.Encyclopedia;
				_previousMouseState = currentMouseState;
				return true;
			}
			...
			// 开始战斗按钮
			int startButtonX = menuButtonsStartX + menuButtonWidth + menuButtonSpacing;
			if (IsPointInRectangle(currentMouseState.X, currentMouseState.Y, startButtonX, menuButtonY, menuButtonWidth, menuButtonHeight))
			{
				LastClickedButton = ButtonType.StartGame;
				_previousMouseState = currentMouseState;
				return true;
			}
			...
			// 教程入口按钮
			int tutorialButtonX = menuButtonsStartX + menuButtonWidth * 2 + menuButtonSpacing * 2;
			if (IsPointInRectangle(currentMouseState.X, currentMouseState.Y, tutorialButtonX, menuButtonY, menuButtonWidth, menuButtonHeight))
			{
				LastClickedButton = ButtonType.Tutorial;
				_previousMouseState = currentMouseState;
				return true;
			}
			...
			int slotScrollAreaHeight = 80;
			int slotScrollAreaX = (windowWidth - slotScrollAreaWidth) / 2;
			int slotScrollAreaY = (int)(scrollAreaY + scrollAreaHeight + menuButtonHeight + 60 + GetFontForText("行动槽数量").MeasureString("行动槽数量").Y + 20);
	...
	("示例敌怪1", () => new Characters.Enemies.示例敌怪1()),
			...
			if (_isDraggingSlotScrollBar)
			{
				int slotScrollAreaWidth = 600;
				int slotScrollAreaHeight = 80;
				int slotScrollAreaY = (int)(scrollAreaY + scrollAreaHeight + 50 + 60 + GetFontForText("行动槽数量").MeasureString("").Y + 20);
				int[] slotOptions = { 5, 10, 15, 20 };
				int slotButtonWidth = 120;
				int slotButtonSpacing = 20;
