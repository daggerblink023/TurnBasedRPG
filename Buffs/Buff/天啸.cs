...
{
    public 天啸(int remainingTurns = 2, int strength = 1) : base("天啸", "（若持有者非魏国武将）暴击伤害抗性提升（20*状态强度）%，速度最小值与速度最大值分别+1\n（若持有者为魏国武将）暴击伤害抗性提升（30*状态强度）%，速度最小值与速度最大值分别+2", remainingTurns, Math.Clamp(strength, 0, 2), false, true)
    {
    ...
    {
        // 魏国武将和非魏国武将的效果不同
        if (character.Faction == Faction.魏)
        {
            // 魏国武将：暴击伤害抗性提升（30*状态强度）%，速度最小值与最大值分别+2
        ...
        {
            // 非魏国武将：暴击伤害抗性提升（20*状态强度）%，速度最小值与最大值分别+1
